...
census <- census %>% mutate(
    turnout = total_votes/X18.and.over.population) %>% data.frame()
...
df <- df %>% mutate(
    VoteByMail = case_when(df$No.Excuse.Absentee == 1 ~ "Any Voter",
                           df$Absentee...Excuse.Required == 1 ~ "Excused Only",
                           df$All.Mail.Voting == 1 ~ "All Voters"),
    AbsenteeStatus = case_when(df$Permanent.Absentee.Status == 1 ~ "Permanent",
                               df$Permanent.Absentee.Status == 0 ~ "Per Case")) %>%
    data.frame()
