...
We also defined some new variables of our own to use in our analysis. The VoterScore variable is a score for each state that conveys how easy it is to vote. Factors of this score were who is allowed to vote by mail, how early people can vote (if early polling locations are available in the state), and how strict ID requirements are. We also added descriptive variables for each community based on demographics such as income and occupation. These variables allowed us to separate voters and states into groups and made it easier to analyze who is affected the most.
...
In contrast with the differences seen in the low income counties, higher income counties do not appear to have any differences in voter turnout based on the flexibility of their laws. The goal with this visualization is to show that high income counties seem less affected by restrictive voting laws and therefore do not have the same extend of voter suppression.
...
![Americans with Disabilities](Data/AwDisabilities.png)
...
The first slider adjusts the hypothetical turnout of disenfranchised felons, as defined by the specific restrictions valid in each state. The second slider adjusts the hypothetical percent of that felon voting population that supports candidate Clinton. In all of the state included in this graph, the state population of disenfranchised felons is greater than the margin of votes for 
